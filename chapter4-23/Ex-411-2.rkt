...
(define (translate c1 c2)
  (local (; Row Content -> [List-of Row]
          ; extract the row match r in c
          (define (findallmatch row_in_c1)
            (local (; Row -> [List-of Row]
                    ; extracts all rows in c which match r.
                    (define (match? row_in_c2)
                      (equal? (last row_in_c1) (first row_in_c2))))
              (filter match? c2)))
          ; Row -> [List-of Row]
          ; attach extra info to r from c2
          (define (combine row1)
            (local ((define match_extras (findallmatch row1))
                    (define (attach row2) (joinlink row1 row2)))
              (map attach match_extras)))
          (define result_by_group (map combine c1)))
    (apply append result_by_group)))
#| complex version 
(foldl (lambda (alor base) (append base alor))
       '()
       (map match c1))
|#
...
(define (joinlink l1 l2)
  (cond [(or (empty? l1) (empty? l2)) (error ERROR_SHORT)]
