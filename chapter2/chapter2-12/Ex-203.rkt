...
; determine tracks that belong to album and
; have been played after date
(define (select-album-date album date alot)
  (cond [(empty? alot) '()]
        [else (if (match? album date (first alot))
                  (cons (first alot) (select-album-date album date (rest alot)))
                  (select-album-date album date (rest alot)))]))
