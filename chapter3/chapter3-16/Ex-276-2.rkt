...
(define (generate-list alos alot)
  (local (; String -> LTracks
          ; get all tracks that belongs to a
          (define (fn a) (select-album a alot))
          )
    ; - IN -
    (map fn alos)))
...
(define (select-album a alot)
  (local (; Track -> Boolean
          ; determine if t belongs to the given album
          (define (fn t) (match? a t))
          )
    ; - IN -
    (filter fn alot)))
...
(define (select-all-album-titles alot) (map track-album alot))
...
(define (create-set alos)
  (local (; String [List-of String] -> [List-of String]
          ; add s into result if s doesn't exist
          (define (fn cur prev) (if (member? cur prev) prev (cons cur prev)))
          )
    ; - IN -
    (foldr fn '() alos)))
