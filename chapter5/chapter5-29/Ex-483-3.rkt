...
; A Sq is an structure:
; (make-sq N N Boolean)
; representation (make-sq x y t) represents a square
; x-coordinate is x , y-coordinate is y,
; if it is threadtening, t should be #f.
(define-struct sq [x y t])
                ...
                (foldl (lambda (cur prev)
                         (if (boolean? prev)
                             (local ((define result (place-queens (add-queen a-board cur) (sub1 n))))
                               (if (boolean? result) result (cons (make-posn (sq-x cur) (sq-y cur)) result)))
                             prev))
                       #false
                       candidates)
                )]))
...
(define (add-queen a-board qp)
  (local ((define (judge sq)
            (local ((define ist (threatening? (make-posn (sq-x qp) (sq-y qp))
                                              (make-posn (sq-x sq) (sq-y sq)))))
              (if ist (make-sq (sq-x sq) (sq-y sq) #t) sq))))
    (map judge a-board)))

; Board -> [List-of QP]
; finds spots where it is still safe to place a queen
(define (find-open-spots a-board)
  (filter (lambda (p) (not (sq-t p))) a-board))
