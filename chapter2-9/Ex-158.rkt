...
; ShotWorld -> ShotWorld
; remove those out of the canvas in w
(define (clear w)
  (cond [(empty? w) '()]
        [else (if (< (first w) 0) (clear (rest w)) (cons (first w) (clear (rest w))))]))

(check-expect (clear '()) '())
(check-expect (clear (cons 10 '())) (cons 10 '()))
(check-expect (clear (cons 20 (cons -10 '()))) (cons 20 '()))
(check-expect (clear (cons 30 (cons 20 (cons -10 '())))) (cons 30 (cons 20 '())))
